...
Reader preflight is ordered and mandatory: (1) read the collection manifest, ETag, and `tombstone_epoch`; (2) completely enumerate and validate all Qdrant tombstone control points, including their UUID, document id, epoch, generation, operation, attempt, fence, and completion proof; (3) compute a deterministic SHA-256 over the canonical, point-id-sorted control-point payloads; (4) exclude every denied document id before graph or vector compatibility fallback. A Qdrant deny record wins for exclusion when it disagrees with the manifest. A manifest tombstone without a matching complete deny record, an unavailable manifest, an incomplete control-point enumeration, or any mismatch/uncertainty fails closed before retrieval. The manifest remains authoritative for normal artifact state; Qdrant is the deny-safety authority only. The deny record may be cleared only by a fresh replace-document generation/attempt/fence CAS that increments `tombstone_epoch`. If the entry was pending and still had a current active pointer, promote that exact pointer into `previous_pointer` before clearing active fields; preserve the newest recovery pointer if one already exists.

Readers retain the manifest ETag, `tombstone_epoch`, and control-point set digest from preflight, perform graph/vector selection, then re-read the manifest and completely re-enumerate the control points before returning results. If the ETag, epoch, or set digest changed, the reader discards the selection and retries from preflight; if revalidation is unavailable or remains inconsistent, it fails closed. Replace-collection cleanup must exclude all `graph_control_point` records from deletion and legacy classification.
