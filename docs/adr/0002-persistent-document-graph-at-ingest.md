...
- `replace-collection` — derive the incoming `document_id` set, apply the same document-level replacement rules only to included documents, and remove omitted document entries from the active collection manifest by the same manifest CAS. Omitted documents become non-discoverable tombstones: their immutable artifact bytes and recovery metadata are retained for audit and recovery, but they must not remain active or reader-discoverable.
