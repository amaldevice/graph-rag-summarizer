...
The durable claim that reserves a pending entry also fences the data plane for the same document and collection. Document-scoped append/replace operations use the entry's `document_fence_token`, issued monotonically by the document claim CAS; the collection `manifest_revision`/collection fencing token is used only for collection-wide operations such as replace-collection and collection setup. On the durable-lock fallback, the corresponding lock token supplies the same scope-specific fence. The claim consists of `operation_id`, `document_generation`, `source_fingerprint`, `pending_attempt_id`, `build_attempt_id`, `pending_version`, and the scope-specific token. Every Qdrant data-plane or control-plane mutation—including collection creation, schema/index creation, control-point writes, upserts, updates, deletes, and cleanup—must go through the collection/document `FenceCoordinator`. The coordinator serializes the operation, validates scope + token + attempt + generation immediately before the call, and rejects stale or expired callers; no direct Qdrant mutation may bypass it. Qdrant payloads and control records carry the generation, attempt identity, and scope-specific fence token. Generation-scoped point IDs or coordinator serialization prevent stale same-ID upserts/deletes. If the backend cannot issue or enforce this fence, the operation fails closed.
...
- `version_ledger` — embedded monotonic version history atomically CAS-updated with this entry; each item is `{version, artifact_key, artifact_digest, operation_id, document_generation, state, reserved_at, updated_at}`.
...
When `active_artifact_key` is null, the active-entry `backend` and `artifact_digest` are null as well. For `tombstoned`, active fields are null, pending fields are cleared, `document_generation`, `source_fingerprint`, `next_version`, and `tombstone_attempt_id` are retained, and `failure_reason` carries the tombstone reason. A tombstoned entry is intentionally non-discoverable and never enters compatibility fallback. Replace-collection writes a durable Qdrant-side tombstone deny control point with valid Qdrant point id `UUID5(UUID_NAMESPACE_URL, "graph-control:tombstone:{collection}:{document_id}")`; the raw `SHA-256(collection + NUL + document_id)` remains an audit payload field. The zero-vector control point carries `graph_control_point=tombstone`, `document_id`, `graph_tombstoned=true`, tombstone `document_generation`, `tombstone_operation_id`, `tombstone_attempt_id`, fence token, and `tombstone_complete=true`. The writer must wait for the write, read the exact point back, and verify the stored payload before considering the tombstone durable; if read-back is missing, mismatched, or incomplete, retry under the same fence or fail closed.

Reader preflight is ordered and mandatory: (1) read the collection manifest and ETag; (2) completely enumerate and validate all Qdrant tombstone control points, including their UUID, document id, generation, operation, attempt, fence, and completion proof; (3) exclude every denied document id before graph or vector compatibility fallback. A Qdrant deny record wins for exclusion when it disagrees with the manifest. A manifest tombstone without a matching complete deny record, an unavailable manifest, an incomplete control-point enumeration, or any mismatch/uncertainty fails closed before retrieval. The manifest remains authoritative for normal artifact state; Qdrant is the deny-safety authority only. The deny record may be cleared only by a fresh replace-document generation/attempt/fence CAS. If the entry was pending and still had a current active pointer, promote that exact pointer into `previous_pointer` before clearing active fields; preserve the newest recovery pointer if one already exists.

Readers retain the manifest ETag and tombstone-set revision from preflight, perform graph/vector selection, then revalidate the manifest and tombstone set before returning results. If either changed, the reader discards the selection and retries from preflight; if revalidation is unavailable or remains inconsistent, it fails closed. Replace-collection cleanup must exclude all `graph_control_point` records from deletion and legacy classification.
...
Version allocation is immutable and ledgered: `version_ledger` is embedded in the collection manifest and atomically CAS-updated with the document entry; object listing is never authoritative. Each ledger item has `version`, `artifact_key`, `artifact_digest`, `operation_id`, `document_generation`, `state`, `reserved_at`, and `updated_at`. Every append reserves `v1`; replacement reserves `max(version_ledger.version)+1` before Qdrant work and never reuses a number. Ledger state is `reserved` while pending, `published_available` or `published_partial` after activation, `failed` for stale/unavailable while bytes remain recovery-only, `superseded` or `burned` when a claim is lost, and `tombstoned` when an omitted document is denied. `next_version` is maintained from this ledger under the same CAS and increases monotonically.
