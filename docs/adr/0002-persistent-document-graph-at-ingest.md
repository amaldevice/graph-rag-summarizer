...
| `tombstoned` | Null | Not authoritative | Non-discoverable tombstone for a document omitted by replace-collection; active fields are cleared, prior recovery metadata stays retained | Skip entry; compatibility fallback |

When `active_artifact_key` is null, the active-entry `backend` and `artifact_digest` are null as well. For `tombstoned`, `active_version`, `active_artifact_key`, `backend`, and `artifact_digest` are all null, `previous_pointer` remains the structured recovery object, `pending_operation_id` / `pending_generation` / `pending_source_fingerprint` / `pending_version` are cleared, `document_generation`, `source_fingerprint`, and `next_version` are retained, `updated_at` is retained, and `failure_reason` carries the tombstone reason.
...
- replace-collection tombstones are written in the same manifest CAS that removes a document from the active set, and the tombstone retains immutable artifact bytes and recovery metadata while clearing the active pointer fields;
