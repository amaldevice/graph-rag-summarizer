...
The durable claim that reserves a pending entry also fences the data plane for the same document and collection. On the normal ETag path, `manifest_revision` is the data-plane fence token; on the durable-lock fallback, the lock's monotonically increasing fencing token is the data-plane fence token. The claim consists of `operation_id`, `document_generation`, `source_fingerprint`, `pending_attempt_id`, `build_attempt_id`, `pending_version`, and that token. Every Qdrant data-plane or control-plane mutation—including collection creation, schema/index creation, control-point writes, upserts, updates, deletes, and cleanup—must go through the collection/document `FenceCoordinator`. The coordinator serializes the operation, validates token + attempt + generation immediately before the call, and rejects stale or expired callers; no direct Qdrant mutation may bypass it. Qdrant payloads and control records carry the generation, attempt identity, and fence token. Generation-scoped point IDs or coordinator serialization prevent stale same-ID upserts/deletes. If the backend cannot issue or enforce this fence, the operation fails closed.
